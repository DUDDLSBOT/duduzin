...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `ADEUS... @${num.split('@')[0]}👋* \n_Se você voltar, não esqueça de trazer a comida frita!!_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5531986283462@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'Qual é o codigo do odioma?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'textnya mana tod?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
               ...
               client.sendMessage(from, 'Este é o número do meu namorado😍, e meu proprietário hehee😁 não fique chateado:*',MessageType.text, { quoted: mek} )
                break
				/*case 'loli':
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Nomes de usuário Onde querido:)?', text, {quoted: mek})
				...
				case 'Escreva':
				case 'Escreva':
					if (args.length < 1) return reply('O que você quer escrever?? Sou eu??')
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'ssweb':
					...
					if (args.length < 1) return reply('O tipo O que um?')
					if (!tipelist.includes(args[0])) return reply('Tipe desktop|tablet|mobile')
					if (args.length < 2) return reply('O url Onde está?')
				...
				case 'ttp':
				case 'tsticker':
					if (args.length < 1) return reply('O texto Onde está?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'clearall':
					...
					reply('Sucesso excluir todo chat :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é você?')
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A você deseja adicionar um gênio??')
					if (args[0].startsWith('08')) return reply('Usar código de país mas')
					try {
				...
				case 'toimg':
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Cadé o texto?')
					teks = body.slice(5)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 ativar, 0 para desativar')
					}
					break
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('O quê é isso ')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já ativado...')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Ativou com sucesso o recurso de boas-vindas neste grupo ✔️')
					} else if (Number(argamassas[0]) === 0) {
